...
{    qDebug()<<__func__;
    ...
    while(1)
    {
        ...
        if(strcmp((char*)packet->data,FLUSH_DATA) == 0)//跳转处理-清空解码器缓冲区
        {
...
void VideoPlayer::run()
{
    ...
    pFormatCtx->interrupt_callback.callback = interrupt_callback;//读取中断回调函数
    ...
    while(1)
    {
        ...
        if (av_read_frame(pFormatCtx, packet) < 0)
        {
            ...
            if( DelayCount>= 300)
            {
                m_videoState.readFinished = true;//用于判断数据回收
                DelayCount = 0 ;
            }
        ...
        DelayCount = 0;//读取成功，重置

        //生产原始数据-放入对应队列-根据流索引判断
    ...
    //回收资源之后,在最后添加读取文件线程退出标志.
    m_videoState.readThreadFinished = true;
    //视频自动结束 置标志位-回收SDL资源，停止回调函数
    //需要避免用户点击停止，SDL回收两次
    if(m_videoState.isNaturalEnd)stop(true);
    //m_playerState = PlayerState::Stop;
}
...
{    qDebug()<<__func__;
    ...
    if( isWait ) //阻塞标志
    {
        ...
        //     SDL_Delay(10);
        // }
        while(m_videoState.audioStream==-1 &&(!m_videoState.readThreadFinished ))//等待读取线程退出
        {
              if(m_videoState.audioStream==-1)
              {
...
{    qDebug()<<__func__;
    ...
    for(;;)
    {
        ...
        while(audio_pkt_size > 0)
        {
            ...
            //一帧一个声道读取数据字节数是 nb_samples , channels 为声道数 2 表示 16 位 2个字节
